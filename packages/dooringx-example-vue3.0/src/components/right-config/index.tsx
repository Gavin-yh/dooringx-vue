...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleChange = (e) => {
      ...
      if (isEdit) {
        const originData: IStoreData = deepCopy(defaultConfig.value.getStoreChanger().getOrigin()!.now)
        ...
        {!current.value && (
          <>
            <div style={{ padding: '20px' }}>
              <el-row style={{ padding: '10px 0 20px 0', fontWeight: 'bold', userSelect: 'none' }}>全局设置</el-row>
              <el-row justify="space-between" style={{ padding: '10px 0' }}>
                <el-col span={6} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  标题
                </el-col>
                ...
                <el-col span={18} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
                  <div class="color-block">
                    <el-color-picker
                      ...
                      v-model={state.value.globalState.containerColor}
                      onChange={(e) => {
                        handleColorChange(e)
                      }}
                    />
            ...
            {configCategory.value.map((tabItem) => (
              <el-tab-pane
                ...
                v-slots={{
                  label: (item = tabItem) => (
                    <>
                      <div class={'tabItem'}>{item.displayName}</div>
                    </>
                  )
                }}
              >
